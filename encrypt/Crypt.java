...
public class Crypt {
    ...
    // generateMAC: generates a message authentication code for the data and key
    //              uses HMAC algorith with SHA-256
    ...
    // verifyMAC: verifies that the given mac matches the mac generated by key and data.
    //            uses HMAC algorith with SHA-256
    public static boolean verifyMAC(byte[] data, byte[] mac, SecretKey key) throws Exception
    {
        Mac gmac = Mac.getInstance("HmacSHA256");
        gmac.init(key);
        
        return Arrays.equals(gmac.doFinal(data), mac);
    }
